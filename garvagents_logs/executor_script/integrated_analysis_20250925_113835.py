...
def plot_detector(detector_name, strain_processed):
    ...
    # FIX: Do not pass ax=axs[1] to q_zoom.plot(), let it create its own axes, then move the image to axs[1]
    try:
        qplot = q_zoom.plot(vmin=0, vmax=15, cmap='viridis')
        # qplot is a matplotlib Figure; get the AxesImage from it and move it to axs[1]
        # Extract the image from the GWpy plot
        qax = qplot.gca()
        # Get the QuadMesh (the pcolormesh) from the axes
        quadmesh = None
        for artist in qax.get_children():
            if hasattr(artist, 'get_array'):
                quadmesh = artist
                break
        # Remove the GWpy axes from its figure and add to ours
        # Instead, let's just clear axs[1] and re-plot using matplotlib's pcolormesh
        # But GWpy's q_transform has a .times and .frequencies and .value
        # So we can use axs[1].pcolormesh directly
        import numpy as np
        t = q_zoom.times.value - GPS_CENTER
        f = q_zoom.frequencies.value
        Z = np.abs(q_zoom.value)
        pcm = axs[1].pcolormesh(t, f, Z.T, vmin=0, vmax=15, cmap='viridis', shading='auto')
        ...
        fig.colorbar(pcm, ax=axs[1], label='Normalized energy')
        plt.close(qplot)  # Close the GWpy-generated plot
    except Exception as e:
        print(f"Error plotting Q-transform for {detector_name}: {e}")
        return
