...
try:
    ...
    strain_H1_full = gw_event.strain('H1')
    # Crop to desired segment
    strain_H1 = strain_H1_full.crop(start_time, end_time)
    # Resample if needed
    if float(strain_H1.sample_rate) != sample_rate:
        strain_H1 = strain_H1.resample(sample_rate)
    times_H1 = strain_H1.sample_times
    print("H1 strain data downloaded and cropped successfully.")

    print(f"Downloading L1 strain data from {start_time} to {end_time} (GPS)...")
    strain_L1_full = gw_event.strain('L1')
    strain_L1 = strain_L1_full.crop(start_time, end_time)
    if float(strain_L1.sample_rate) != sample_rate:
        strain_L1 = strain_L1.resample(sample_rate)
    times_L1 = strain_L1.sample_times
    print("L1 strain data downloaded and cropped successfully.")
