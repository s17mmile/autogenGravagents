...
# --- Task 1: Download H1 and L1 strain data for GW150914 using gwpy ---
def fetch_strain_gwpy(detector, start, end, max_retries=3, wait_sec=5):
    """Fetch strain data from LOSC using gwpy with retries."""
    channel_map = {'H1': 'H1:GWOSC-4KHZ_R1_STRAIN', 'L1': 'L1:GWOSC-4KHZ_R1_STRAIN'}
    if detector not in channel_map:
        raise ValueError(f"Unknown detector: {detector}")
    for attempt in range(1, max_retries + 1):
        try:
            print_progress(f"Attempt {attempt}: Downloading {detector} strain data from GWOSC via gwpy...")
            # FIX: Explicitly specify source='gwosc' to avoid nds2 dependency
            strain = GwpyTimeSeries.get(channel_map[detector], start, end, source='gwosc')
...
strain_H1 = None
strain_L1 = None
gps_time = 1126259462.4  # GW150914 GPS time (hardcoded for robustness)
event_name = "GW150914"

try:
    print_progress(f"Using GW150914 GPS time: {gps_time}")

    duration = 32  # seconds
    start = int(gps_time) - duration // 2
    end = start + duration

    # Fetch strain data using gwpy
    strain_H1_gwpy = fetch_strain_gwpy('H1', start, end)
    strain_L1_gwpy = fetch_strain_gwpy('L1', start, end)

    # Convert gwpy TimeSeries to pycbc TimeSeries for compatibility
    strain_H1 = TimeSeries(strain_H1_gwpy.value, delta_t=strain_H1_gwpy.dt.value, epoch=strain_H1_gwpy.t0.value)
    strain_L1 = TimeSeries(strain_L1_gwpy.value, delta_t=strain_L1_gwpy.dt.value, epoch=strain_L1_gwpy.t0.value)
