...
for det, ts in filtered_gw190521_strain.items():
    if ts is None:
        ...
        continue
    try:
        print(f"Generating spectrogram for {det} around the merger...")
        ts_window = ts.crop(GW190521_GPS - spec_window, GW190521_GPS + spec_window)
        # The correct signature is spectrogram(stride, fftlength, **kwargs)
        fftlength = 0.125
        overlap = 0.0625
        stride = fftlength - overlap  # stride = window step size
        spec = ts_window.spectrogram(stride, fftlength)
        fig = spec.plot(norm='log', vmin=1e-24, vmax=1e-21)
        ax = fig.gca()
        ax.set_title(f"GW190521 {det} | Spectrogram ±{spec_window}s around Merger")
        ax.set_xlabel("Time [s] (GPS)")
