...
def fetch_strain_gwpy(detector, start, end, max_retries=3, wait_sec=5):
    ...
    for attempt in range(1, max_retries + 1):
        try:
            print_progress(f"Attempt {attempt}: Downloading {detector} strain data from LOSC via gwpy...")
            # FIX: Remove cache=True, which is not a valid argument for get()
            strain = GwpyTimeSeries.get(channel_map[detector], start, end)
