...
def gwpy_to_pycbc(ts):
    # Use standard constructor as per database guidance
...
for det, data in data_pycbc.items():
    if data is not None:
        try:
            ...
            # --- CRITICAL FIX: Dynamically adjust seg_len to <= half data duration ---
            data_duration = len(data) * data.delta_t
            # Use a default seg_len of 4s, but ensure it's <= half the data duration and at least 2s
            max_seg_len = max(2, int(data_duration // 2))
            default_seg_len = 4
            seg_len = min(default_seg_len, max_seg_len)
            nperseg = int(seg_len / data.delta_t)
            # If data is very short, further reduce seg_len
            if nperseg > len(data):
                nperseg = len(data) // 2
                seg_len = nperseg * data.delta_t
            if nperseg < 2:
                raise ValueError(f"Data for {det} too short for PSD estimation (nperseg={nperseg})")
            psd = welch(data, seg_len=nperseg)
            # Interpolate and truncate as before
            psd = interpolate(psd, len(data))
            trunc_len = int(4 / data.delta_t)
            psd = inverse_spectrum_truncation(psd, trunc_len)
            psds[det] = psd
            print(f"PSD estimated for {det} (seg_len={seg_len}s, nperseg={nperseg}).")
        except Exception as e:
