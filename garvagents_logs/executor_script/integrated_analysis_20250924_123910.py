...
for det, ts in filtered_gw190521_strain.items():
    ...
    try:
        ...
        fftlength = 0.125
        stride = fftlength * 0.5  # 50% overlap
        spec = ts_window.spectrogram(stride, fftlength)
        plot = spec.plot(figsize=(10, 4))
        ax = plot.gca()
        ax.set_title(f"{det} Spectrogram | GW190521 | ±{spec_window}s around Merger")
        ax.set_xlabel("Time (s) since GPS {:.1f}".format(GW190521_GPS))
        ax.set_ylabel("Frequency (Hz)")
        ax.set_xlim(GW190521_GPS - spec_window, GW190521_GPS + spec_window)
        ax.set_ylim(30, 400)
        plt.tight_layout()
