...
"""
    ...
    def _detect_runtime_errors(self, stdout: str, stderr: str, return_code: int) -> tuple:
        """
        Simple error detection - capture everything and let LLM decide
        Returns: (has_errors: bool, error_indicators: List[str])
        """
        error_indicators = []
        
        # Always capture stderr if it exists (except harmless warnings)
        if stderr and stderr.strip():
            # Filter out only the most harmless warnings
            harmless_patterns = [
        ...
        # Always capture stdout - let LLM determine if it contains errors
        ...
        # If we have any output at all, let the LLM analyze it
...
[Brief explanation of the fixes applied and why they should work]"""
    ...
    Based on the error and any previous failed attempts above, analyze this failure and provide a corrected version that:

    1. Fixes the specific error that occurred
    2. Uses a DIFFERENT approach than any previous attempts (if applicable)
    3. Only uses available packages listed above
    4. Includes robust error handling to prevent similar failures
    5. Is fundamentally different in approach if this is attempt #{self.debug_attempt_count} or higher
