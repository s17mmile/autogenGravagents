# filename: gw150914_analysis_final_qtransform.py
...
def save_plot(filename):
    """Saves the plot with tight layout to the specified filename."""
    plt.tight_layout()
    try:
...
# Task 3: Q-Transform
q_transform_h1 = QTransform(h1_filtered)
q_transform_l1 = QTransform(l1_filtered)
...
save_plot(f'template_{mass}_task5.png')
        ...
        print(f'Warning: Template generation failed for mass {mass}.')
...
print('Best fit templates:')
for mass, (h1_peak, l1_peak) in best_fit.items():
    print(f'Mass: {mass} | H1 Peak: {h1_peak:.2f} | L1 Peak: {l1_peak:.2f}')