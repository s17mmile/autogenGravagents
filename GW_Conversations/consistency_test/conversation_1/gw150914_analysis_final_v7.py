# filename: gw150914_analysis_final_v7.py
...
try:
    ...
    for mass in masses:
        ...
        template, _ = waveform.get_fd_waveform(approximant='IMRPhenomD', mass1=mass, mass2=mass, spin1z=0, spin2z=0, f_lower=20, delta_f=1.0)
        if isinstance(template, tuple) and len(template) > 0:
            waveform_object = template[0]  # Get the waveform object from the tuple
            if hasattr(waveform_object, 'duration') and waveform_object.duration > 0.2:
                # Pad or truncate to match data length
                waveform_object = waveform_object.crop(0, min(waveform_object.duration, h1_pycbc.duration))
                templates.append(waveform_object)
                save_plot(waveform_object.sample_times, waveform_object, f'Template {mass}M_sun vs Strain', 'GPS Time (s)', 'Amplitude', f'template_{mass}M_sun.png', f'Template {mass}M_sun')
        else:
...
try:
    ...
    h1_psd = psd.estimate(h1_pycbc, segment_length=4, overlap=2)
    l1_psd = psd.estimate(l1_pycbc, segment_length=4, overlap=2)
