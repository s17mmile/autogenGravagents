# filename: gw150914_analysis_corrected.py
import os
import numpy as np
...
# Skip resampling if the current rate matches the desired rate
if h1_filtered.sample_rate != 4096:
    h1_resampled = h1_filtered.resample(4096)
else:
    h1_resampled = h1_filtered

if l1_filtered.sample_rate != 4096:
...
try:
    h1_psd = estimate(h1_filtered)
    l1_psd = estimate(l1_filtered)
except Exception as e:
