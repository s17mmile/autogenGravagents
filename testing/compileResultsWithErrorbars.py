...
def createComparisons(results, output_dir=os.path.join(os.path.dirname(__file__), "charts"), hist_bins = 200, show=False):
    ...
    for idx, stat_name in enumerate(stat_names):
        ...
        yerr = None
        if stat_name == "Avg Score":
            lower_errs = []
            upper_errs = []
            for pair_label in pair_names:
                data = raw_stats["scores"][pair_label]
                std = float(np.std(data, ddof=1)) if len(data) > 1 else 0.0
                mean_val = float(np.mean(data)) if len(data) > 0 else 0.0
                lower_errs.append(min(std, mean_val))
                upper_errs.append(std)
            yerr = np.array([lower_errs, upper_errs], dtype=float)
        bars = ax.bar(x, values, yerr=yerr, capsize=4, color=[colors[i % len(colors)] for i in range(n_pairs)])
