...
dt = 0.5  # increased time step for simulation
...
plt.subplot(3, 1, 1)
plt.plot(np.arange(0, len(heights) * dt, dt), heights, label='Height (m)', color='blue')
...
plt.subplot(3, 1, 2)
plt.plot(np.arange(0, len(velocities) * dt, dt), velocities, label='Velocity (m/s)', color='orange')
plt.ylabel('Velocity (m/s)')
plt.legend()
plt.grid()

plt.subplot(3, 1, 3)
plt.plot(np.arange(0, len(accelerations) * dt, dt), accelerations, label='Acceleration (m/s^2)', color='green')
plt.xlabel('Time (s)')
