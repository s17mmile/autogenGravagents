...
h = 1.5  # Reduced height of the fence in meters
...
def simulate_trajectory(v0, angle, h):
    """
    Simulates the trajectory of a softball considering linear air resistance.
    Parameters:
    ...
    while y >= 0 and t <= max_time:
        # Calculate drag force (linear model)
        F_d = c_W * rho * A * v0
        # Update velocities
        ax = -F_d * (vx / np.sqrt(vx**2 + vy**2)) / m
        ay = -g - (F_d * (vy / np.sqrt(vx**2 + vy**2))) / m
        vx += ax * dt
        vy += ay * dt
        # Update position
        x += vx * dt
