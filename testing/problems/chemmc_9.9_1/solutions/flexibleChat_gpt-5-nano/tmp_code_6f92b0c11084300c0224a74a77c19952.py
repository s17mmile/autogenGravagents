...
def is_valid_hartree_eV(Eh_eV: float,
                       expected: float = 27.211386245988,
                       rel_tol: float = 1e-6,
                       abs_tol: float = 1e-12) -> bool:
    """Return True if Eh_eV matches the expected value within tolerances."""
    return math.isclose(Eh_eV, expected, rel_tol=rel_tol, abs_tol=abs_tol)


def assert_valid_hartree_eV(Eh_eV: float,
                          expected: float = 27.211386245988,
                          rel_tol: float = 1e-6,
                          abs_tol: float = 1e-12) -> None:
    """Raise ValueError if Eh_eV is not within tolerances of expected."""
    if not is_valid_hartree_eV(Eh_eV, expected, rel_tol, abs_tol):
        raise ValueError(
            f"Hartree energy in eV {Eh_eV:.12f} differs from expected {expected:.12f} within tolerance"
        )
...
if __name__ == "__main__":
    ...
    # Non-fatal validation example: useful for demos and unit tests
    print("Validation against expected value:", is_valid_hartree_eV(Eh_eV))
    # Optional strict check (uncomment to enable strict runtime enforcement):
