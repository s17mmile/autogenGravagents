# filename: numerov_harmonic_oscillator_probability_v2.py
import numpy as np
...
def calculate_forbidden_probability():
    # Define spatial grid from -5 to 5 with step 0.1
    ...
    prob_forbidden = simps(psi_norm[forbidden_mask]**2, x[forbidden_mask])

    return prob_forbidden

if __name__ == "__main__":
    prob = calculate_forbidden_probability()
    print(f"Probability of being in the classically forbidden region for v=0 state: {prob:.6f}")
