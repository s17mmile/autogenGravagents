...
def numerov_wavefunction(x):
    ...
    print(f'Normalization factor: {norm}')  # Debugging output
    if norm == 0:
...
# Visualization of the wave function and potential
plt.figure(figsize=(10, 6))
plt.plot(x, psi**2, label='Wave Function Probability Density', color='blue')
plt.plot(x, potential(x)/(np.max(potential(x)))*0.1, label='Potential Energy (scaled)', color='red')
plt.axhline(y=E0/(np.max(potential(x)))*0.1, color='green', linestyle='--', label='Total Energy')
plt.title('Harmonic Oscillator Wave Function and Potential Energy')
plt.xlabel('Position (x)')
plt.ylabel('Probability Density / Potential Energy')
plt.legend()
plt.grid()
plt.savefig('harmonic_oscillator_wave_function.png')
plt.close()