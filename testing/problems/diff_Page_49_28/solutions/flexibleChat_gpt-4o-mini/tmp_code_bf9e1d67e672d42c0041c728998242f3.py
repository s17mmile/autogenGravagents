import numpy as np
from scipy.optimize import fsolve

# This function computes the time T at which the solution of the initial value problem first reaches 3.99.

def find_time_to_reach_value(target_value=3.99, initial_guesses=[0.1, 1.0, 2.0]):
    # Define the function based on the rearranged equation
    def equation(T):
        return target_value * ((1 + T)**4 + 4 * np.exp(4 * T)) - 4 * np.exp(4 * T)

    for guess in initial_guesses:
        T_solution, info, ier, msg = fsolve(equation, guess, full_output=True)
        if ier == 1:  # Check if the solution converged
            return T_solution[0]
        else:
            print(f'Initial guess {guess} did not converge: {msg}')  

    raise ValueError('None of the initial guesses led to a converged solution.')

# Call the function and print the result
try:
    T = find_time_to_reach_value()
    print(f'The time T at which the solution first reaches 3.99 is approximately: {T:.4f}')
except ValueError as e:
    print(e)