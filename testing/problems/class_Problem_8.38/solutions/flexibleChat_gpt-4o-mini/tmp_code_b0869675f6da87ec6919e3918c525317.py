...
def calculate_velocity_perihelion(r_E, semi_major_axis):
    return math.sqrt(G * M * (2 / r_E - 1 / semi_major_axis))


def calculate_velocity_aphelion(r_V, semi_major_axis):
    return math.sqrt(G * M * (2 / r_V - 1 / semi_major_axis))
...
def calculate_delta_v():
    ...
    v_perihelion = calculate_velocity_perihelion(r_E, semi_major_axis)
    print(f'Velocity at perihelion (v_perihelion): {v_perihelion:.2f} m/s')

    # Step 3: Calculate the velocity at aphelion (Venus's orbit)
    v_aphelion = calculate_velocity_aphelion(r_V, semi_major_axis)
    ...
    Delta_v2 = v_V - v_aphelion  # Second burn
