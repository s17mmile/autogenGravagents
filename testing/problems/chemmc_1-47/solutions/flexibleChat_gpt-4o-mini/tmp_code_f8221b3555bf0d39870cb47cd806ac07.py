def calculate_uncertainty_in_momentum(delta_x):
    """
    Calculate the uncertainty in momentum (Delta p) based on the uncertainty in position (Delta x).
    ...
    """
    if delta_x <= 0:
        raise ValueError('Uncertainty in position must be positive.')
...
# Output the result
print(f'The calculated uncertainty in momentum (Delta p) is: {Delta_p:.3e} kg m/s')