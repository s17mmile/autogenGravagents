...
# Compute Vm from EOS and from density
Vm_VdW_L = solve_vm_vdw(p, T, a, b, R)
Vm_VdW_m3 = Vm_VdW_L / 1000.0


def main():
    # Density-derived molar volume
    Vm_m3_from_density = M_kg_per_mol / rho_given
    Vm_L_from_density = Vm_m3_from_density * 1000.0

    # EOS-derived Vm and derived properties
    Vm_vdw_L = Vm_VdW_L
    Vm_vdw_m3 = Vm_VdW_m3
    ...
    return {
        ...
        "Vm_vdw_mol_per_mol": Vm_VdW_m3,  # kept for compatibility if needed
...
if __name__ == "__main__":
    results = main()
    print("Vm from density: {:.6f} L/mol, {:.6e} m3/mol".format(results["Vm_density_L_per_mol"], results["Vm_density_m3_per_mol"]))
    print("Vm from van der Waals: {:.6f} L/mol, {:.6e} m3/mol".format(results["Vm_vdw_L_per_mol"], results["Vm_vdw_m3_per_mol"]))
    print("rho_EoS: {:.1f} kg/m^3, rho_given: {:.1f} kg/m^3, rel_error: {:.2%}".format(results["rho_EoS"], results["rho_given"], results["rho_rel_error"]))
