...
dotm = 900.0
tb = 100.0
F = dotm * 4000.0           # thrust during burn (N)
...
def derivatives(t, h, v, rho, burn, g_func):
    # Mass depending on burn phase with clamping to mf to avoid dipping below mf
    m = m0 - dotm * t
    if burn:
        if m < mf:
            m = mf
...
def apex_interpolate(t_prev, h_prev, v_prev, t_next, h_next, v_next, dt):
    # Local interpolation for zero crossing of velocity within the last step
    if v_next == v_prev:
        return h_prev
    frac = (0.0 - v_prev) / (v_next - v_prev)
    return h_prev + frac * (h_next - h_prev)
...
def burn_phase(rho, dt, g_func):
    ...
    while t < tb:
        ...
        t, h, v = rk4_step(t, h, v, step, rho, True, g_func)
    return t, h, v
...
def coast_phase(t_burn, h_burn, v_burn, rho, dt, g_func):
    ...
    t_prev, h_prev, v_prev = t, h, v
    while True:
        t_next, h_next, v_next = rk4_step(t, h, v, dt, rho, False, g_func)
        if v_next <= 0.0:
