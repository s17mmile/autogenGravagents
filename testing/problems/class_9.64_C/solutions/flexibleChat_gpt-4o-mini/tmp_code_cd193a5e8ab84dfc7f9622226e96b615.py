...
def air_density(height):
    ...
    # This is a rough approximation; actual air density decreases more complexly with altitude.
...
# Further increased dt to reduce the number of iterations
dt = 2.0  # Time step (s)
...
max_iterations = 2000
max_height_threshold = 50000  # Adjusted maximum height threshold (m)
...
for iteration in range(max_iterations):
    ...
    if time < constants['burn_time']:
        ...
        if current_mass < 0:
            current_mass = 0  # Prevent negative mass
    else:
        # After fuel burn
        thrust = 0
    ...
    acceleration = net_force / current_mass if current_mass > 0 else 0
...
print(f'Simulation parameters: Time step = {dt} s, Total simulation time = {len(results["time"])} seconds')
...
plt.xlabel('Time (s)')
plt.ylabel('Height (m)')
plt.xlim(0, max(results['time']))
plt.ylim(0, max(results['height']) * 1.1)  # Set y-limit to 10% above max height
plt.grid()
plt.legend()
