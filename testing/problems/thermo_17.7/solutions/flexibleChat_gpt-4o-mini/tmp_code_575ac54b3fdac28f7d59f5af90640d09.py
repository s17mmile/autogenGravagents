...
def calculate_flow_rate(radius, P1, P2, viscosity, length):
    ...
    Parameters:
    radius (float): Radius of the tube in meters.
    P1 (float): Input pressure in Pascals.
    P2 (float): Output pressure in Pascals.
    ...
    """
    if P1 <= P2:
        raise ValueError('Input pressure must be greater than output pressure.')
...
if mass_flow_rate == 0:
    time_seconds = float('inf')  # Infinite time if no flow
else:
    time_seconds = mass_CO2 / mass_flow_rate

# Output the results
print(f'Flow Rate: {flow_rate:.6f} m³/s')
print(f'Mass Flow Rate: {mass_flow_rate:.6f} kg/s')
print(f'Time the cylinder lasts: {time_seconds:.2f} seconds')