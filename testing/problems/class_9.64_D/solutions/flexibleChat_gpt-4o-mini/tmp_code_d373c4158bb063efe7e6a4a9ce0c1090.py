...
max_time = 120  # Reduced maximum simulation time in seconds
time_step = 0.5  # Decreased time step for better performance
num_steps = int(max_time / time_step)  # Number of steps
...
for t in range(num_steps):
    ...
    rho = 10**(0.11 - 0.05 * (height / 1000))  # Convert height to km for the formula
    ...
    if t % 10 == 0:  # Print every 10 steps
        print(f'Time: {current_time:.1f} s, Height: {height:.1f} m, Velocity: {velocity:.1f} m/s, Mass: {mass:.1f} kg')
