...
class ScientificInterpreterAgent:
    ...
    def __init__(self):
        ...
        self.api_key = "sk-XfLVggCdoE9tO5AChw7fKQ"
...
[Brief explanation of the fixes applied and why they should work]"""
    ...
    def analyze_and_fix_error(self, failed_code: str, error_message: str, 
                         stderr_output: str, original_query: str, 
                         available_packages: Dict[str, str], debug_history: List[Dict] = None) -> Dict:
        """Analyze error and generate fixed code with learning from previous attempts"""
        print(f"\n[DEBUGGER AGENT] Analyzing execution failure...")
        
        # Extract key error information
        error_lines = stderr_output.split('\n') if stderr_output else []
        main_error = error_lines[-1] if error_lines else error_message

        print(f"[DEBUGGER] Querying database for error solutions...")
        error_query = f"{main_error} fix solution workaround"
        documentation = self.query_documentation(error_query, n_results=3)

        doc_context = ""
        if documentation:
            doc_context = "\n=== RELEVANT DOCUMENTATION FROM DATABASE ===\n"
            for i, doc in enumerate(documentation, 1):
                doc_context += f"\nDocument {i}: {doc['title']}\n"
                doc_context += f"{doc['content'][:500]}...\n"
            doc_context += "\n=== END DOCUMENTATION ===\n"
        else:
            doc_context = "\n(No specific documentation found for this error in database)\n"
        ...
        if debug_history and len(debug_history) > 0:
            previous_attempts_context = f"\n=== PREVIOUS FAILED ATTEMPTS ({len(debug_history)}) ===\n"
            
            for i, prev_attempt in enumerate(debug_history, 1):
                attempt_error = prev_attempt.get('original_error', 'Unknown error')
                attempt_explanation = prev_attempt.get('explanation', 'No explanation provided')
                attempt_analysis = prev_attempt.get('error_analysis', 'No analysis provided')
                
                previous_attempts_context += f"\nAttempt {i}:\n"
                previous_attempts_context += f"- Error encountered: {attempt_error}\n"
                previous_attempts_context += f"- Fix attempted: {attempt_explanation}\n"
                previous_attempts_context += f"- Analysis: {attempt_analysis[:200]}...\n"
                
                # Extract key approaches that failed
                failed_approaches.append(attempt_explanation.lower())
            
            previous_attempts_context += f"\n=== END PREVIOUS ATTEMPTS ===\n"
            previous_attempts_context += f"\nCRITICAL: The above {len(debug_history)} approaches all FAILED with similar errors.\n"
            previous_attempts_context += f"You must try a FUNDAMENTALLY DIFFERENT approach. Do not repeat these methods:\n"
            
            for i, approach in enumerate(failed_approaches, 1):
                previous_attempts_context += f"{i}. {approach}\n"
            
            previous_attempts_context += f"\nRequirement: Your solution must be structurally different from all previous attempts.\n"
    ...
    DEBUG ATTEMPT #{self.debug_attempt_count}:

    The documentation above (if found) shows correct patterns and anti-patterns for handling similar errors.
    Based on the error, documentation, and any previous failed attempts, analyze this failure and provide a corrected version that:

    1. Uses the guidance from the documentation to fix the specific error that occurred
    2. Applies the correct methods and patterns shown in the anti-pattern documentation
    3. Uses a DIFFERENT approach than any previous attempts (if applicable) 
    3. Only uses available packages listed above
    4. Includes robust error handling to prevent similar failures
    5. Follows the exact function signatures and parameter types from the documentation

    CRITICAL: If the documentation shows that a method or import doesn't exist (like "bandpass does NOT exist in pycbc.filter"), you MUST use the alternative approaches suggested in the documentation.

    Focus on finding the ROOT CAUSE of the error rather than just handling the symptoms.

    Please provide your error analysis, corrected code, and explanation of the fixes.
    """
        
        print("[DEBUGGER AGENT] Sending debug request to LLM...")
        llm_response = self.call_llm(debug_prompt)
        
        # Parse the debug response
        error_analysis, fixed_code, explanation = self._parse_debug_response(llm_response)
        ...
        print(f"[DEBUGGER AGENT] Generated fix ({len(fixed_code)} characters)")
        return result
    ...
    def debug_execution_loop(self, integration_result: Dict, execution_result: Dict,
        ...
        STDOUT Output (last 500 chars): {stdout_output[-500:] if stdout_output else 'No output'}
                ...
                debug_result = self.analyze_and_fix_error(
                    current_code, error_message, full_error_context,
                    original_query, available_packages, debug_history  # Pass debug_history
                )
