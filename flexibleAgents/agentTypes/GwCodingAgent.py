...
def GwCodingAgent(chat, name = "GwCodingAgent") -> ConversableAgent:
	systemMessage = f"""
		...
		All plots should be saved to disk in the working directory with appropriate names. Any code written should include print statements to indicate task progress. Make sure to properly indent your code, especially if using try/except blocks, but only use these blocks when it's really necessary! All necessary libraries (gwpy, pycbc) are installed and up to date and all necessary data is available - code failure should not be diagnosed as improper installation or lack of data, only incorrect implementation!
