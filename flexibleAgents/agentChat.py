...
class flexibleAgentChat(QObject):
    ...
    def sendMessageToGUI(self, sender: ConversableAgent, message, recipient: ConversableAgent, silent = False):
        ...
        # HumanAgent works a little differently due to different message format, message string is wrapped into dict for GUI passing
        if sender == self.humanAgent:
            newMessage = {"name": sender.name, "content": {"message": message}}
        else:
            newMessage = message

        print(f"sendMessageToGUI called with sender: {sender}, recipient: {recipient}, message: {message}, silent: {silent}")
        self.signals.outgoingMessage.emit(newMessage)
